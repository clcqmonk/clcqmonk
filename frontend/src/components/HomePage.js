...
const HomePage = ({ user, onLogin, onRegister }) => {
  ...
  const fetchWinners = async () => {
    try {
      const response = await fetch(`${backendUrl}/api/winners`);
      const data = await response.json();
      setWinners(data);
    } catch (error) {
  ...
  const categories = ['All', 'House', 'Car', 'Land', 'Electronics', 'Experience'];
  ...
  return (
    <div>
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            <div className="text-center">
              <div className="text-3xl font-bold text-yellow-400">{stats.total_raffles || 0}</div>
              <div className="text-white/80">Active Raffles</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-green-400">{stats.total_participants || 0}</div>
              <div className="text-white/80">Happy Participants</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-red-400">{formatPrice(stats.total_prize_value || 0)}</div>
              <div className="text-white/80">Total Prize Value</div>
            </div>
          </div>
      ...
      {winners.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8">
            <h3 className="text-2xl font-bold text-white mb-6 text-center">🏆 Recent Winners</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {winners.slice(0, 6).map((winner, index) => (
                <div key={index} className="bg-white/20 rounded-lg p-4 text-center">
                  <div className="text-lg font-semibold text-yellow-400">{winner.user_name}</div>
                  <div className="text-white text-sm">{winner.prize_title}</div>
                  <div className="text-green-400 font-bold">{formatPrice(winner.prize_value)}</div>
                  <div className="text-white/60 text-xs">
                    {new Date(winner.announced_date).toLocaleDateString()}
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
        <div className="bg-red-500/20 border border-red-500/30 rounded-lg p-4 text-center">
          <p className="text-white font-semibold">
            ⚠️ Must be 18+ to participate • Gamble responsibly • Terms & Conditions apply
          </p>
        </div>
      </section>
      ...
      {/* Purchase Modal */}
      {showPurchaseModal && selectedRaffle && user && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full">
            <div className="p-6">
              ...
              <div className="bg-blue-50 p-4 rounded-lg mb-6">
                <div className="text-sm text-gray-500 mb-1">Purchasing as</div>
              ...
              <form onSubmit={handlePurchase} className="space-y-4">
                <div>
                  ...
                  />
                  <div className="text-xs text-gray-500 mt-1">Maximum 10 tickets per purchase</div>
                </div>
                ...
                <div className="bg-green-50 p-4 rounded-lg text-sm text-green-700">
                  ...
                  <ul className="space-y-1 text-xs">
                    <li>• Instant ticket confirmation via email</li>
                    <li>• Secure ticket IDs generated</li>
                    <li>• Automatic entry into the draw</li>
                    <li>• Winner notification if selected</li>
                  </ul>
                </div>
